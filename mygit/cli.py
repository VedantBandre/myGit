...
def parse_args():
    ...
    diff_parser.add_argument('--cached', action='stored_true')
...
def _diff(args):
    ...
    if args.cached:
        ...
        if not args.commit:
            # If no commit was provied, diff from HEAD
            oid = base.get_oid('@')
            tree_from = base.get_tree(oid and base.get_commit(oid).tree)
    else:
        ...
        if not args.commit:
            ...
            tree_form = base.get_index_tree()

    result = diff.diff_trees(tree_form, tree_to)
    sys.stdout.flush()
    sys.stdout.buffer.write(result)
